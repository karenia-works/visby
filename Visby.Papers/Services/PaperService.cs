...
namespace Karenia.Visby.Papers.Services
{
    public class PaperService
    {
        ...
        {
            var ps = await _context.Papers.FromSqlRaw("select * from Papers where keyword <@ '{0}'", Keywords).ToListAsync();
            return ps;
        }
        public async Task<List<Paper>> GetPaperAuthor(List<String> authors)
        {
            var ps = await _context.Papers.FromSqlRaw("select * from Papers where Author <@ '{0}'", authors).ToListAsync();
            return ps;
        }
